...
public class CombineResult {
	...
	public CombineResult(HashMap<String, HashMap<String, Double>> featureWeightPerClass, String resultPath,boolean weighted,boolean sum) {
		super();
				...
				try {
				    ...
				    while((buf=reader.readLine())!=null){
						if(buf.split("\t").length>2){
							String predict_label = buf.split("\t")[buf.split("\t").length-1];
							String class_name = "";
							...
							if(map.containsKey(line_count)){
								map.put(line_count, map.get(line_count)+"\t"+class_name+"/"+belief);
							}else{
								...
								map.put(line_count, content+class_name+"/"+belief);
							}
						}
						line_count++;
					}
