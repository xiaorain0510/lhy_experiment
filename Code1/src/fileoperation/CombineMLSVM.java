...
public class CombineMLSVM {
	...
	public void Excute(String input,String output){
		...
		readBiggestResult(input, output);
	}
	
	public CombineMLSVM(int attribute_count) {
		super();
				...
				try {
				    ...
				    while((buf=reader.readLine())!=null){
						if(buf.split("\t").length>2){
							...
							String label_with_value = "";
							for(int j=attribute_count+3;j<buf_list.length;j++){
								if(buf_list[j].contains(predict_label)){
									label_with_value = buf_list[j];
								}
							}
							String class_name = "";
							for(int i=0;i<label_with_value.split("/").length-1;i++){
								if(i==0){
									class_name = label_with_value.split("/")[i];
								}else{
									class_name = class_name+"/"+label_with_value.split("/")[i];
								}
							}
							double belief = Double.valueOf(label_with_value.split("/")[label_with_value.split("/").length-1]);
							belief = Math.abs(belief);
							belief = 1/belief;
							String lineContent = "";
							if(classIdContain(class_name)){
								lineContent = belief+"\t"+classIdGet(class_name);								
							}else{
								lineContent = belief+"\t"+22;
							}
							if(map.containsKey(line_count)){
								map.put(line_count,map.get(line_count)+"\t"+lineContent);
							}else{		
								if(classIdContain(buf_list[attribute_count+1])){
									map.put(line_count,classIdGet(buf_list[attribute_count+1])+"\t"+lineContent);
								}else{
									map.put(line_count, "22\t"+lineContent);									
								}
							}
						}
						line_count++;
...
//		}
	...
	public boolean classIdContain(String class_name){
		Iterator iter = classId.entrySet().iterator();
		while(iter.hasNext()){
			Map.Entry entry = (Map.Entry) iter.next();
			String key = (String) entry.getKey();
			int val = (int) entry.getValue();
			if(class_name.contains(key)){
				return true;
			}else if(key.contains(class_name)){
				return true;
			}
		}
		return false;
	...
	public int classIdGet(String class_name){
		Iterator iter = classId.entrySet().iterator();
		while(iter.hasNext()){
			Map.Entry entry = (Map.Entry) iter.next();
			String key = (String) entry.getKey();
